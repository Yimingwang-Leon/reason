...
#### E4 探针判决(2026-06-11 02:00,~$5)
- **三门全 FAIL**:crypt 前向格式留出 2/40=5%;bit EV 格式留出 0/30=0%;防遗忘 32/40=80%(<90%,小训还伤了锚)。
- 含义(证据分级 B+,探针训练制式 60步/LR8e-5/续退火态):新格式在**未见难题**上无法被贪心执行——"枚举-验证"教得会背诵(equation 100% 是**已训题**复现),但**搜索过程的泛化执行**在难尾上 ≈0。今晚的 +114 bit/+91 crypt oracle 离线为真,但模型背不动它们。
...
### run-013 [FIRING] 2026-06-11
- 诊断依据:run-012 holdout 解码(770 题分层)= cipher/gravity/numeral/unit 100%×4、bit 82.7%、eq 76.4%、**crypt 0%(105/110 撞 7680 上限,无限枚举)** → 真凶 = forward-crypt 毒格式;LR 3.5e-4 与 replay 无罪(A 级证据)。
- 配方:run-011 语料重建(worktree@d77bf2c,18034 行,总数与 config 对账分毫不差)+ 已过闸 replay 4059 → 22093 行;唯一变量 = 公开配对增量(replay + LR 3.5e-4)。
- 闸:4 车道全票(replay 完整性/合并完整性 22093 全量扫/发车审计 = 隔离专家;重建保真+毒格式 161/161 = 本地确定性字节比对,因子代理会话额度中断)。
- 已知偏离(B 级注记):run-011 实际 ep1 因事故退化 plain-CE,本次全程 curriculum;run-013 主 bit 1601/eq 566 为构建真值(台账旧文 1602/732 系记忆误差,总数 18034 对账为准)。
- 预登记预测:中心 0.855-0.86;P(显示≥0.86)≈40-45%;**≤0.85 = replay+LR 配对增量在我们语料上干净证伪,战役止于 run-011**。
- 发车:python -m src.train_tinker --run-name run-013 --num-epochs 2 --curriculum --lr 3.5e-4 --budget-usd 38 --usd-per-step 0.05(692 步,预计 $33.4,钱包 ~$40-43,余量 1.2-1.29×,402 暂停-续跑路径已实测)

#### run-013 结果(2026-06-11 夜)
- **LB = 0.84**(53567387)。预登记证伪线 ≤0.85 第二次触发,且低于 run-011 基线 1pp。
- 干净归因(本次是真单束:语料字节级=run-011+replay,毒源四车道闸确认绝迹):**{replay 4059 + LR 3.5e-4 + curriculum-ep1} 在我们语料上 = -1pp,公开配对增量(mohamedamr 0.85→0.86)不可迁移,A 级证据**。注:run-011 的 ep1 因事故是 plain-CE,系第三个微小变量,无法再拆(不资助)。
- 战役总账:run-011 0.85 = 自训最佳;replay/LR/新格式三杆全部实测排除;**预承诺生效:停止付费实验,final = kienngx 0.86 + run-011 0.85**。

### 融合三票(免费,当日配额)[FIRING] 2026-06-11 夜
- 预登记(rl/PLAN.md §04 原案,T-C 亲本修正 012→013:013 无毒且同语料族,合并对更干净):
  T-C = 011⊕013 ΔW-TIES 等权,中心 0.84-0.85;T-A1 = kienngx-0.86 宿主+λ0.25 注入 011ΔW,中心 0.86(=无效奖),P(0.87)≈10%;T-A2 = λ0.5+DARE p0.9,高方差版。
- 硬前置:kienngx r/alpha/键集/dtype 验明正身;五重诊断闸(键/dtype/范数±5%/能量ρ≥0.90/NaN)任一 FAIL=废票。
- 现金 $0;读法:任一 ≥0.87 才改变结局(终选地板=0.86 在账),0.85/0.86 = 无效奖不外推。
